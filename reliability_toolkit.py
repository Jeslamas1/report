...
Quality-of-life updates in this revision
----------------------------------------
• New `diag` command to print interpreter path/versions and dependency status.
• Global `--debug` flag (or RELIABILITY_DEBUG=1) prints full tracebacks on errors.
• Matplotlib is now optional/lazy—non-plotting commands run without it installed.
• Keeps safe-exit behavior so notebooks/REPLs don’t show `SystemExit: 0`.
• Self-tests expanded (no behavior changes to existing tests).

Install (recommended)
---------------------
python -m pip install numpy pandas
# Optional (for plots & Excel export):
python -m pip install matplotlib openpyxl xlsxwriter
# Optional (better optimizers/statistics):
...
try:
    ...
    # Hard requirement: give a useful message and bail early.
    msg = (
        "This program requires numpy and pandas.\n"
        "Install with:\n"
        "  python -m pip install numpy pandas\n\n"
        f"Underlying import error: {e}"
    )
    print(msg)
    # Avoid noisy SystemExit in interactive hosts
    sys.exit(1)

# Try SciPy (optional). Fallback to numpy-only methods if missing.
try:
...
def _in_interactive_session() -> bool:
    ...
    if os.environ.get("PYTHONINSPECT") == "1": return True
    return False
...
def _debug_enabled(args: Optional[argparse.Namespace] = None) -> bool:
    if args is not None and getattr(args, "debug", False):
        return True
    return os.environ.get("RELIABILITY_DEBUG", "0") not in ("0", "", "false", "False")
...
def load_table(path: str) -> pd.DataFrame:
    ...
    if ext in (".xlsx", ".xlsm", ".xls"):
        try:
            ...
            raise RuntimeError(
                "Reading Excel requires 'openpyxl'. Install with:\n"
...
class Distribution:
    name = "base"
    def nll(self, t: np.ndarray, failed: np.ndarray) -> float: raise NotImplementedError
    def fit(self, t: np.ndarray, failed: np.ndarray) -> FitResult: raise NotImplementedError
    def R(self, t: np.ndarray, **params) -> np.ndarray: raise NotImplementedError
    def h(self, t: np.ndarray, **params) -> np.ndarray: raise NotImplementedError
    def B(self, p: float, **params) -> float: raise NotImplementedError
...
class Weibull(Distribution):
    ...
    def fit(self, t: np.ndarray, failed: np.ndarray) -> FitResult:
        t = np.asarray(t, dtype=float); failed = np.asarray(failed, dtype=int)
        assert np.all(t > 0), "Times must be > 0"

        def nll_wrap(x):
            b, e = x
            if b <= 0 or e <= 0: return 1e100
            return self.nll(t, failed, b, e)

        if SCIPY_AVAILABLE:
            x0 = np.array([1.5, np.median(t)])
            bounds = [(1e-6, 100.0), (1e-9, 1e12)]
            res = optimize.minimize(nll_wrap, x0, bounds=bounds, method="L-BFGS-B")
            b, e = res.x; loglik = -nll_wrap(res.x); cov = None
        ...
        # Fallback: grid + coordinate descent
        ...
        for _ in range(40):
            ...
            e0 = e_candidates[int(np.argmin([nll_wrap((b0, ec)) for ec in e_candidates]))]
            b_candidates = np.geomspace(max(b0*0.6, 1e-3), b0*1.6, 25)
            b0 = b_candidates[int(np.argmin([nll_wrap((bc, e0)) for bc in b_candidates]))]
    ...
    def R(self, t: np.ndarray, beta: float, eta: float) -> np.ndarray:
        t = np.asarray(t, dtype=float); return np.exp(-(t/eta)**beta)
    def h(self, t: np.ndarray, beta: float, eta: float) -> np.ndarray:
        t = np.asarray(t, dtype=float); return (beta/eta) * (t/eta)**(beta-1.0)
...
class Exponential(Distribution):
    ...
    def nll(self, t: np.ndarray, failed: np.ndarray, lam: float) -> float:
        f = failed.astype(bool); t_f = t[f]; t_s = t[~f]
    ...
    def fit(self, t: np.ndarray, failed: np.ndarray) -> FitResult:
        t = np.asarray(t, dtype=float); failed = np.asarray(failed, dtype=int)
        assert np.all(t > 0), "Times must be > 0"
        total_time = np.sum(t); r = failed.sum()
        lam = r / total_time if total_time > 0 else float("nan")
        return FitResult(self.name, {"lambda": float(lam), "MTBF": float(1.0/lam) if lam > 0 else float("inf")},
                         -self.nll(t, failed, lam), len(t), int(r), int((1-failed).sum()), None)

    def R(self, t: np.ndarray, lam: float, **_) -> np.ndarray:
        t = np.asarray(t, dtype=float); return np.exp(-lam*t)
...
class Lognormal(Distribution):
    ...
    def fit(self, t: np.ndarray, failed: np.ndarray) -> FitResult:
        ...
        for _ in range(40):
            ...
            mu = mu_grid[int(np.argmin([nll_wrap((mg, s)) for mg in mu_grid]))]
            s_grid = np.linspace(max(s*0.6, 1e-3), s*1.6, 25)
            s = s_grid[int(np.argmin([nll_wrap((mu, sg)) for sg in s_grid]))]
...
def expected_spares_poisson(failure_rate_per_hour: float, hours: float, service_level: float = 0.95) -> int:
    ...
    k, cdf = 0, math.exp(-lam); prob = cdf
    while cdf < service_level and k < 10_000:
        k += 1; prob *= lam/k; cdf += prob
    return k
...
def plot_weibull_probability(ax, t: np.ndarray, failed: np.ndarray, fit: FitResult):
    ...
    if x.size == 0:
        ...
        return
    xs, ys = ecdf(x); F = ys
    y = np.log(-np.log(1 - F)); xlog = np.log(xs)
    ...
    ax.set_xlabel("ln(time)"); ax.set_ylabel("ln(-ln(1-F))"); ax.legend(); ax.grid(True, which="both", ls=":")

def plot_hazard(ax, dist: Distribution, fit: FitResult, tmax: float):
    t = np.linspace(1e-6, tmax, 300); h = dist.h(t, **fit.params)
    ax.plot(t, h); ax.set_xlabel("Time"); ax.set_ylabel("Hazard rate h(t)"); ax.grid(True, ls=":")

def pareto_chart(ax, labels: List[str], counts: List[int]):
    order = np.argsort(-np.array(counts))
    labels = [labels[i] for i in order]; counts = [counts[i] for i in order]
    cum = np.cumsum(counts) / max(sum(counts), 1)
    ax.bar(labels, counts)
    ax2 = ax.twinx(); ax2.plot(range(len(labels)), cum, marker="o"); ax2.set_ylabel("Cumulative %")
    ax.set_ylabel("Count"); ax.set_title("Pareto of Failures")
    ax.set_xticklabels(labels, rotation=45, ha="right"); ax.grid(True, axis="y", ls=":")
...
def fit_distribution(dist_name: str, t: np.ndarray, failed: np.ndarray) -> FitResult:
    ...
    return DIST_MAP[key].fit(t, failed)

def summarize_fit(f: FitResult, timescale: str = "hours") -> pd.DataFrame:
    rows = [
        {"Metric": "Distribution", "Value": f.dist},
...
def export_report(out_path: str, fit: FitResult, t: np.ndarray, failed: np.ndarray,
    ...
    with pd.ExcelWriter(out_path, engine="xlsxwriter") as writer:
        ...
        pd.DataFrame({"Time": t, "Failed": failed}).to_excel(writer, index=False, sheet_name="Data")
        ...
        if fit.dist == "weibull":
            ...
            fig.tight_layout(); p1 = os.path.join(charts_dir, "weibull_prob.png")
        else:
            plot_hazard(ax, dist, fit, tmax=float(np.percentile(t, 95)))
            fig.tight_layout(); p1 = os.path.join(charts_dir, "hazard.png")
        fig.savefig(p1, dpi=160); plt.close(fig)  # type: ignore

        p2 = None
        if pareto is not None:
            labels, counts = pareto
            fig2, ax2 = plt.subplots(figsize=(6,4))  # type: ignore
            pareto_chart(ax2, labels, counts)
            fig2.tight_layout(); p2 = os.path.join(charts_dir, "pareto.png")
            fig2.savefig(p2, dpi=160); plt.close(fig2)  # type: ignore

        ws = writer.sheets["Summary"]
        ws.insert_image(1, 4, p1, {"x_scale": 0.9, "y_scale": 0.9})
        if p2: ws.insert_image(22, 4, p2, {"x_scale": 0.9, "y_scale": 0.9})
        if meta:
...
def action_pareto(args) -> int:
    ...
    labels = counts.index.astype(str).tolist(); nums = counts.values.astype(int).tolist()
    out = pd.DataFrame({"Category": labels, "Count": nums}); print(out.to_string(index=False))
    if args.plot:
        _need_matplotlib()
        fig, ax = plt.subplots(figsize=(8,5))  # type: ignore
        pareto_chart(ax, labels, nums); fig.tight_layout(); plt.show()  # type: ignore
    return 0
...
def action_report(args) -> int:
    ...
    meta = {"source_file": os.path.abspath(args.csv), "distribution": args.dist, "time_col": args.time_col, "failed_col": args.failed_col}
...
class ComponentModel:
    name: str; dist: str; params: Dict[str, float]
...
def eval_block(block: Dict[str, Any], t: float) -> float:
    ...
    if btype == "component":
        cm = ComponentModel(block["name"], block["dist"], block["params"]); return cm.R(t)
    if btype == "series":
        return series_reliability([eval_block(it, t) for it in block.get("items", [])])
    if btype == "parallel":
        return parallel_reliability([eval_block(it, t) for it in block.get("items", [])])
    raise ValueError(f"Unknown block type: {btype}")

def action_system(args) -> int:
    config = load_system_config(args.config)
    t = float(config.get("time", 1000.0)); blocks = config.get("blocks", [])
    R_blocks = [eval_block(b, t) for b in blocks]
    R_sys = R_blocks[0] if len(R_blocks) == 1 else series_reliability(R_blocks)
...
def action_gui(_args=None) -> int:
    try:
        ...
        print("Tkinter not available:", e); return 1

    root = tk.Tk(); root.title("Reliability Toolkit"); root.geometry("820x600")
    csv_path = tk.StringVar(); time_col = tk.StringVar(value="Time"); failed_col = tk.StringVar(value="Failed"); dist_name = tk.StringVar(value="weibull")
    ...
    def browse():
        ...
        if p:
            csv_path.set(p)
            try:
                df = load_table(p); cols = list(df.columns)
                time_col.set(cols[0] if cols else "Time")
                if "Failed" in cols: failed_col.set("Failed")
    ...
    text = tk.Text(frame, height=25); text.grid(row=3, column=0, columnspan=4, sticky="nsew", pady=10)
    frame.columnconfigure(1, weight=1); frame.rowconfigure(3, weight=1)
    root.mainloop(); return 0

# --------------------------------- diag ------------------------------------ #
def action_diag(_args=None) -> int:
    import platform
...
def action_selftest(_args=None) -> int:
    ...
    t = np.array([10.0, 10.0, 10.0]); f = np.array([1, 1, 1])
    ...
    try:
        try:
            fit_distribution("nope", np.array([1.0]), np.array([1]))
            failures.append("Unknown distribution did not raise")
        except ValueError:
            pass
    except Exception as e:
        failures.append(f"Unknown distribution test crashed: {e}")

    # Survival monotonicity (Weibull R(t) should decrease)
    w = Weibull()
    R1 = w.R(np.array([10.0]), beta=1.5, eta=1000.0)[0]
    R2 = w.R(np.array([100.0]), beta=1.5, eta=1000.0)[0]
    if not (R1 > R2):
        failures.append("Weibull survival not monotone decreasing")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--debug", action="store_true", help="Print full tracebacks on error (or set RELIABILITY_DEBUG=1)")
    sub = p.add_subparsers(dest="cmd")

    sp = sub.add_parser("fit", help="Fit a life distribution to failure/censored data")
    sp.add_argument("--csv", required=True); sp.add_argument("--time-col", required=True)
    sp.add_argument("--failed-col", default="Failed"); sp.add_argument("--censor-col", default="Failed")
    sp.add_argument("--censor", type=int, default=None)
    ...
    sp2.add_argument("--csv", required=True); sp2.add_argument("--category-col", required=True)
    sp2.add_argument("--count-col", default=None); sp2.add_argument("--plot", action="store_true")
    sp2.set_defaults(func=action_pareto)

    sp3 = sub.add_parser("report", help="End-to-end fit + Excel report with charts")
    sp3.add_argument("--csv", required=True); sp3.add_argument("--time-col", required=True)
    sp3.add_argument("--failed-col", default="Failed"); sp3.add_argument("--dist", choices=list(DIST_MAP.keys()), default="weibull")
    sp3.add_argument("--out", required=True); sp3.add_argument("--pareto-category-col", default=None)
    ...
    sp4.add_argument("--config", required=True); sp4.set_defaults(func=action_system)

    sp5 = sub.add_parser("gui", help="Launch a minimal GUI"); sp5.set_defaults(func=action_gui)

    sp6 = sub.add_parser("selftest", help="Run built-in deterministic tests"); sp6.set_defaults(func=action_selftest)

    sp7 = sub.add_parser("diag", help="Show environment diagnostics"); sp7.set_defaults(func=action_diag)
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    try:
        ...
        print("Interrupted by user"); return 130
    except Exception as e:
